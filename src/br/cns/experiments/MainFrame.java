...
import br.cns.experiments.nodePositions.PowerLawNodePositions;
import br.cns.experiments.setup.ApplicationSetup;
import br.cns.experiments.setup.MetricExperimentSetup;
import br.cns.experiments.setup.ParameterExperimentSetup;
import br.cns.experiments.som.SomExperimentDialog;
import br.cns.models.Barabasi;
...
import br.cns.models.Gilbert;
import br.cns.models.KRegular;
...
import br.cns.persistence.ApplicationSetupDaoXml;
import br.cns.persistence.ComplexNetworkDaoXml;
import br.cns.persistence.ResultSetDaoXml;
import br.cns.persistence.SimpleEdgeDataSet;
import br.cns.transformations.DegreeMatrix;
import br.cns.util.FirstKindBesselFunction;
import br.cns.util.FormatUtils;
...
public class MainFrame implements ActionListener {
	private static final String LABORATORIO_DE_REDES_COMPLEXAS = "Laboratório de Redes Complexas UFPE-UPE :: CNS - Complex Network Simulator v1.0 beta";
	...
	private void initialize() throws ParseException {
		...
		paneCanvas.add("Distribuição de Grau dos Nós", histogramDegreeDistrib);
	...
	public void saveConfigurations(ApplicationSetup setup, boolean exit) {
		try {
			...
			JOptionPane.showMessageDialog(canvas, "Não foi possível gravar as configurações.");
		}
		if (!exit
				&& JOptionPane.showConfirmDialog(canvas,
						"Deseja realizar uma nova simulação com as alterações nas configurações?",
	...
	private JMenuBar createMenu(ActionListener al) {
		...
		menu.getAccessibleContext().setAccessibleDescription("Menu principal da aplicação.");
		...
		menuItem.getAccessibleContext().setAccessibleDescription("Criar uma nova simulação de redes complexas.");
		menu.add(menuItem);
		...
		menuItem.getAccessibleContext().setAccessibleDescription("Finaliza a aplicação.");
		menu.add(menuItem);

		menu = new JMenu("Ajuda");
		menu.setMnemonic(KeyEvent.VK_J);
		menu.getAccessibleContext().setAccessibleDescription("Sobre a aplicação.");
		menuBar.add(menu);
		...
		menuItem.setMnemonic(KeyEvent.VK_S);
		menuItem.getAccessibleContext().setAccessibleDescription("Informações sobre a aplicação.");
		menu.add(menuItem);
	...
	public void executeSimulation(boolean forceNewNodes) {
		...
		try {
			...
			JOptionPane.showMessageDialog(canvas, "Número de nós inválido.");
		}
	...
	private void atualizarResultados(String[] header, Object[][] data, ComplexNetwork network) {
		...
		if (histogramDegreeDistrib == null) {
			histogramDegreeDistrib = new XYGraphCanvas("Distribuição de Grau dos Nós",
	...
	/**
	 * Atualiza os dados do painel gráfico quando uma linha é selecionada na
	...
	/**
	 * Atualiza os dados da rede na visualização gráfica do grafo e nos
	 * histogramas.
	 * 
	 * @param network
	 *            Nova rede.
	...
	private void fecharAplicacao() {
		if (JOptionPane.showConfirmDialog(canvas, "Confirma o encerramento da aplicação?",
				LABORATORIO_DE_REDES_COMPLEXAS, JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			try {
				...
				JOptionPane.showMessageDialog(this.btnNewNodes,
						"Não foi possível salvar a rede. Escolha outro nome de arquivo.");
			}
		}
	}
	...
	private void abrirRede() {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			try {
				...
				JOptionPane.showMessageDialog(this.btnNewNodes, "Arquivo inválido!");
			}
		}
	}
	...
	private void exportTableToTxt() {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			try {
				...
				JOptionPane.showMessageDialog(this.btnNewNodes,
						"Não foi possível salvar os resultados. Escolha outro nome de arquivo.");
			}
		}
	}
	...
	private void saveImage() {
		...
		try {
			...
			JOptionPane.showMessageDialog(canvas, "Não foi possível gravar a imagem.");
		}
	}
	...
	/**
	 * Método acessor para obter o valor do atributo histogram.
	 * 
	...
	/**
	 * Método acessor para alterar o valor do atributo histogram.
	 * 
	 * @param histogram
