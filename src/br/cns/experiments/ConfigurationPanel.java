...
public class ConfigurationPanel extends JDialog implements ActionListener {
	...
	public ConfigurationPanel(JFrame frame, MainFrame parent, String title) {
		...
		tabbedPane.addTab("Configurações gerais", null, generalConfigs,
		...
		tabbedPane.addTab("Mapas auto-organizáveis", null, metricsSOM,
		...
		button.addActionListener(this);
		panelBotoes.add(button);
		button = new JButton("Restaurar Padrão");
		button.addActionListener(this);
		panelBotoes.add(button);
	...
	private JPanel createSOM() {
		...
		JLabel label = new JLabel("Número de iterações:");
		...
		label = new JLabel("Raio de vizinhança (%):");
	...
	private void salvar() {
		...
		try {
			...
			JOptionPane.showMessageDialog(tableMetrics,
					"Parâmetro de mapas auto-organizáveis inválido!");
		}
				...
				if (mapQtdeVariations.get(parameter.getType().getModel()) == 1) {
					JOptionPane
							.showMessageDialog(tableMetrics,
									"É permitido variar apenas um parâmetro para cada modelo!");
				} else {
	...
	public void actionPerformed(ActionEvent ae) {
		if (ae.getActionCommand().equals("Salvar")) {
			...
			setVisible(false);
			dispose();
		} else if (ae.getActionCommand().equals("Restaurar Padrão")) {
